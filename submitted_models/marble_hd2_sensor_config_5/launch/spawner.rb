...
def rosExecutables(_name, _worldName)
  ...
  <executable name='robot_description'>
    <command>roslaunch --wait marble_hd2_sensor_config_5 description.launch world_name:=#{_worldName} name:=#{_name}</command>
  </executable>
  <executable name='topics'>
    <command>roslaunch --wait marble_hd2_sensor_config_5 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name}</command>
  </executable>
  HEREDOC
end
